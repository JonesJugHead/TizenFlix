...
/** Point d'entree */
export function transformSerie() {
  hideSerieUnwanted();
  buildDetailHero();
  buildPlayerSection();
  buildRelatedRow();
...
function buildPlayerSection() {
  ...
  const langs = [];
  if (episodeData.vostfr.length) langs.push({ key: 'vostfr', label: 'VOSTFR', eps: episodeData.vostfr });
  if (episodeData.vf.length)     langs.push({ key: 'vf',     label: 'VF',     eps: episodeData.vf });
...
function parseEpList(block, suffix, article) {
  ...
  items.forEach(item => {
    ...
    container.querySelectorAll('a[onclick]').forEach((a, i) => {
      ...
      servers.push({ label: lbl, url });
    });

    if (servers.length === 0) return;

    eps.push({ label, epNum: parseInt(epNum, 10) || 0, servers });
  });
